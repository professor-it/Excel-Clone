...
describe('createStore', () =>{
  ...
  test('should call subscriber fuction', () => {
    store.subscribe(hendler)

    store.dispatch({type: 'ADD'})

    expect(hendler).toHaveBeenCalled()
    expect(hendler).toHaveBeenCalledWith(store.getState())
  })

  test('should NOT call sub if unsubscribe', () => {
    const sub = store.subscribe(hendler)

    sub.unsubscribe()
    store.dispatch({type: 'ADD'})

    expect(hendler).not.toHaveBeenCalled()
  })
